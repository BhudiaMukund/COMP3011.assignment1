 ...
 * returned. The logging below records size, duration and token
 * counts only - enough to diagnose latency and cost, nothing sensitive.
 */
@Service
@Profile("!stub")
public class OpenAiTranscriptionService implements TranscriptionService {
    ...
    public TranscriptionResult transcribe(byte[] audio, String filename, String contentType) {
        ...
        if (!properties.hasApiKey()) {
            throw new TranscriptionFailedException(
                    "Speech-to-text credentials are not configured.", null);
        }

        long startNanos = System.nanoTime();
        try {
            OpenAiTranscription response = restClient.post()
                    .uri(properties.transcriptionsPath())
                    .header(HttpHeaders.AUTHORIZATION, "Bearer " + properties.apiKey())
                    .contentType(MediaType.MULTIPART_FORM_DATA)
                    .body(buildMultipartBody(audio, filename))
                    .retrieve()
            ...
            log.info("Transcription completed: bytes={}, contentType={}, durationMs={}, "
     ...
     *
     * The anonymous ByteArrayResource subclass supplies filename, which is
     * required for the upstream service to treat the part as a file upload
     * rather than a plain form field. Without it the request will be rejected.
     */
     ...
     * fields later. @JsonProperty maps the provider's snake_case onto camelCase
     * Java without applying a global naming strategy, that could corrupt the
